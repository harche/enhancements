# KEP-5970: DRA Driver Validation Test Suite
...
- [Alternatives](#alternatives)
  - [Option A: Separate <code>kubernetes-sigs/dra-validation</code> repository](#option-a-separate-kubernetes-sigsdra-validation-repository)
...
existing validation tests only cover API-level CRUD, and the e2e tests use
a fake driver that can't be swapped out for a real one.

This KEP adds a driver-agnostic validation test suite that any DRA driver
...
validation tests added in v1.35 don't help either; they just verify that
the API server accepts CRUD on the DRA types, without ever exercising the
driver. In practice, a vendor could pass every existing validation test
...
any validation tests they can run against their fork to know if they've
broken something.
...
- Define the minimum set of behaviors that any validated DRA driver must
  implement.
- Create an upstream, driver-agnostic validation test suite that validates
...
- Enable distributions (OpenShift, EKS, GKE, etc.) to run the upstream
  suite in their CI to verify driver validation.
- Build on existing Kubernetes e2e test infrastructure (`test/e2e/dra/`,
  `drautils`, `e2evalidation`, Ginkgo/Gomega).
...
Create a validation test suite at `test/e2e/dra/validation/` in the
...
[Validation Test Scope](#validation-test-scope) table below.
...
```bash
go test ./test/e2e/dra/validation \
...
upstream DRA driver validation suite in my CI pipeline against real drivers
...
I want a clear set of validation tests that tell me exactly what behaviors
...
- **Single-node vs multi-node:** The minimum validation suite requires only
...
- **Dependency on a running driver:** Unlike the existing API CRUD validation
...
- **Version skew:** The DRA gRPC plugin interface has both `v1` and `v1beta1`
  versions. The validation suite should test against the API version supported
...
The following table defines the minimum validation behaviors:
...
Tests are tagged with `[DRADriverValidation]` and labeled with `DRADriverValidation`.

**Location:** `kubernetes/kubernetes/test/e2e/dra/validation/`

**Invocation:**

```bash
# Run the full DRA driver validation suite
go test ./test/e2e/dra/validation \
...
ginkgo -v -focus="\[DRADriverValidation\]" \
  --label-filter="DRADriverValidation" \
  ./test/e2e/dra/validation -- \
  --driver-name=<driver-name> \
  --device-class=<device-class-name>
```
...
are required; the validation suite builds on top of the existing
infrastructure.

##### Unit tests

The validation suite is itself a test suite. Unit tests will cover:
- Test configuration parsing (driver name, device class flags)
- Helper functions for resource verification

##### Integration tests

Not applicable. The validation suite is an e2e test suite that runs against
a live cluster.

##### e2e tests

The validation suite *is* the e2e test deliverable. It will be validated by
running against:
...
- Validation test suite implemented with all 7 minimum behaviors
...
- Community consensus that the minimum validation scope is correct
...
validation suite tests against whatever version the kubelet under test
...
- [x] Other (treat as last resort)
  - Details: The test suite produces standard Ginkgo pass/fail output and
    JUnit XML reports. A passing suite indicates the driver is validated.
...
- **Maintenance burden:** The validation suite has to be kept up to date
...
- **False sense of security:** Passing validation doesn't mean a driver
...
### Option A: Separate `kubernetes-sigs/dra-validation` repository
...
means duplicating test infrastructure (`drautils`, `e2evalidation`, etc.).
...
fake driver (failure injection, call counting). A separate validation
...
- A CI job in testgrid to run the validation suite against the upstream
